...
def set_welcome(chat_id, file, caption):
    welcome.update_one({"chat_id": chat_id}, {
                       "$set": {"file": file, "caption": caption}}, upsert=True)
...
def set_welcome_clean(chat_id, clean):
    welcome.update_one({"chat_id": chat_id}, {
                       "$set": {"clean": clean}}, upsert=True)


def set_welcome_captcha(chat_id, captcha):
    welcome.update_one({"chat_id": chat_id}, {
                       "$set": {"captcha": captcha}}, upsert=True)


def get_welcome(chat_id):
    return welcome.find_one({"chat_id": chat_id}) or {"chat_id": chat_id, "file": [], "captcha": False, "mode": True, "caption": "Hi {first_name}, welcome to {title}", "clean": False}
