...
def digital_ocean(cc, exp_mo, exp_year, cvv):
    ...
    response = requests.post('https://cloud.digitalocean.com/graphql',
    ...
    try:
        ...
        pass
    intent_secret = response.json(
    )['data']['createPaymentMethodV2']['intent_secret']
    ...
    response = requests.get(
        f'https://api.stripe.com/v1/payment_intents/{intent}', params=params, headers=headers_s)
    resp = response.json()
    if "status" in resp:
