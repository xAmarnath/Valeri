...
from ._handler import newMsg, newCall
...
async def _seedr_account(msg):
    ...
    if token is None:
        ...
        SEEDR = Seedr(
            str(token), callbackFunc=lambda token: update_seedr(user, token))
        user = SEEDR.getSettings()
        user_caption = '<b>Seedr Account Connected</b>\n'
        user_caption += '<b>Username:</b> <code>' + \
            user['account']['username'] + '</code>\n'
        user_caption += '<b>Email:</b> <code>' + \
            user['account']['email'] + '</code>\n'
        user_caption += '<b>ID:</b> <code>' + \
            user['account']['user_id'] + '</code>\n'
        if int(user['account']['premium']) == 1:
            user_caption += '<b>Premium:</b> Yes\n'
        else:
            user_caption += '<b>Premium:</b> No\n'
        await msg.reply(user_caption, buttons=[Button.inline('Logout', data='logout')], parse_mode='HTML')
...
async def _seedr_login(call):
    ...
    await call.edit(msg.format(code['user_code']), parse_mode='html', buttons=[
...
async def noAccount(msg, call=False):
    ...
    if not msg.is_private:
        return await msg.reply("⚠️ You don't have any account logged in.",
...
async def _seedr_addtorrent(msg):
    ...
    if add['result'] == 'not_enough_space_added_to_wishlist':
        return await add_msg.edit("⚠️ Not enough space to add torrent, added to wishlist, SIZE: <code>{}</code>".format(
            add['wt']['size']), parse_mode='HTML')
    await add_msg.edit("✅ Torrent added.\n<b>Name:</b>  <code>{}</code>".format(
        add['title']), parse_mode='HTML', buttons=None)
...
async def _seedr_usage(msg):
    ...
    usage_caption += '<b>Bandwidth:</b> <code>{}</code>/<code>{}</code>\n'.format(
        human_readable_size(usage['bandwidth_used']), human_readable_size(usage['bandwidth_max']))
    usage_caption += '<b>Space:</b> <code>{}</code>/<code>{}</code>\n'.format(
        human_readable_size(usage['space_used']), human_readable_size(usage['space_max']))
...
async def _seedr_getfiles(msg):
    ...
    if 'error' not in response:
        if 'name' in response:
            ...
            markup = []
            markup.append([Button.inline("🔗 Download Link", data=f"download_{id}"), Button.inline(
                "🗑 Delete", data=f"delete_{id}")])
            ...
            markup.append(
                [Button.inline("Open in media player", data=f"open_{id}")])
