...
def upload_file(file_path, file_name):
    ...
    headers = {
        ...
        'Dropbox-API-Arg': '{"path":"/uploads/'+file_name+'","mode":{".tag":"overwrite"}}',
    }
    try:
...
def get_download_url(file_name):
    ...
    try:
        ...
        return {"error": "File not found"}
    r = r.json()
    if r.get("error_summary") and "shared_link_already_exists" in str(r.get("error_summary")):
...
@newMsg(pattern="(drop|dropbox|upload|space|list|geturl|delfile|filesearch|dropboxhelp)")
async def _dropbox(e):
    ...
    if cmd == "upload" or cmd == "drop":
        ...
        os.remove(file)
        endTime = time.time()
        dropbox = f"**Dropbox upload complete in {endTime - startTime:.2f} seconds.**\n" \
                  f"**Name**: {file_name}\n" \
            f"**Size:** {human_readable_size(r['size'])} \n" \
            f"**File ID:** {r['id']} \n" \
            f"**Path:** {r['path_display']}"
        await e.reply(dropbox, buttons=[
            Button.url(text="Download", url=url)
        ...
        if len(r) > 4096 or "-l" in e.text:
            with io.BytesIO(str.encode(r)) as out_file:
                ...
                return await e.client.send_file(
                    e.chat_id, out_file, force_document=True, reply_to=e, caption="`List of files in the folder:`")
        await e.reply(r)
    elif cmd == "space":
        ...
        await dp.edit(dropbox, parse_mode="md", link_preview=False, buttons=[
            Button.url("Download \U0001F4E2", url)
        ])
    elif cmd == "delfile":
        if len(e.text.split(" ")) > 1:
            file_name = e.text.split(" ")[1]
            ...
            for file in files:
                btns.append(Button.inline(
                    file, data="deldp_{}x{}".format(len(buttons), column)))
                column += 1
                if column == 3:
                    buttons.append(btns)
                    btns = []
            ...
            if len(btns) > 0:
                buttons.append(btns)
            await e.reply("Choose a file to delete.", buttons=buttons, parse_mode="markdown", link_preview=False)
    elif cmd == "filesearch":
        if len(e.text.split(" ")) > 1:
            file_name = e.text.split(" ")[1]
        else:
        ...
        for file in files:
            btns.append(Button.inline(
                file, data="showfile_{}x{}".format(len(buttons), column)))
            column += 1
            if column == 3:
                buttons.append(btns)
                btns = []
        ...
        if len(btns) > 0:
            buttons.append(btns)
        await e.reply("Search Results", buttons=buttons, parse_mode="markdown", link_preview=False)
...
async def _delfilecq(e):
    ...
    file_name = (await e.get_message()).reply_markup.rows[int(row)].buttons[int(column)].text
    r = delete_file(file_name)
...
async def _showfilecq(e):
    ...
    dropbox = '''**DROPBOX FILE**\n**📁 Path:** /uploads/{path}\n**📄 Name:** {name}\n**📄 Size:** {size}'''.format(
        path=file_name, name=file_name, size=human_readable_size(int(size)))
    await e.edit(dropbox, parse_mode="md", link_preview=False, buttons=[
        Button.url("Download \U0001F4E2", url)
    ])
